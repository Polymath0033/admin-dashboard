<script lang="ts">
	import { users, edit } from '../../stores/useStore';
	import { get } from 'svelte/store';

	let { userId, toggleModal } = $props();
	let user = get(users).find(user => user.id === userId);
	...
	const handleSubmit = (event: Event) => {
		...
		edit(userId, { name, email, role, status });
		toggleModal();
	};
</script>

<form onsubmit={handleSubmit} class="px-6 py-4 z-[1000] relative">
	<div class="space-y-4">
		<div>
